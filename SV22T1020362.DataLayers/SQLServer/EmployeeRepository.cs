    ...
    public class EmployeeRepository : BaseRepository, IEmployeeRepository
    {
        ...
        public async Task<PagedResult<Employee>> ListAsync(PaginationSearchInput input)
        {
            ...
            var sql = @"SELECT COUNT(*)
                        FROM Employees
                        WHERE FullName LIKE @searchValue OR Email LIKE @searchValue OR Address LIKE @searchValue;

                        SELECT EmployeeID, FullName, BirthDate, Address, Phone, Email, Photo, ISNULL(IsWorking, 0) AS IsWorking
                        FROM Employees
                        WHERE FullName LIKE @searchValue OR Email LIKE @searchValue OR Address LIKE @searchValue
                        ORDER BY FullName
                        OFFSET @offset ROWS
        ...
        {
            using var connection = GetConnection();
            var sql = "SELECT EmployeeID, FullName, BirthDate, Address, Phone, Email, Photo, ISNULL(IsWorking, 0) AS IsWorking FROM Employees WHERE EmployeeID = @id";
