    ...
    public static class PartnerDataService
    {
        ...
        public static async Task<int> AddCustomerAsync(Customer data)
        {
            ...
            if (!await customerDB.ValidateEmailAsync(data.Email))
                throw new ArgumentException("Email đã được sử dụng bởi khách hàng khác.");
        ...
        public static async Task<bool> UpdateCustomerAsync(Customer data)
        {
            ...
            if (string.IsNullOrWhiteSpace(data.Email))
                throw new ArgumentException("Email không được để trống.");
            if (!await customerDB.ValidateEmailAsync(data.Email, data.CustomerID))
                throw new ArgumentException("Email đã được sử dụng bởi khách hàng khác.");
        ...
        public static async Task<int> AddShipperAsync(Shipper data)
        {
            ...
            if (string.IsNullOrWhiteSpace(data.ShipperName))
                throw new ArgumentException("Tên người giao hàng không được để trống.");
